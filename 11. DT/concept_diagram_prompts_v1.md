...
>
> **v2.1 변경 (2026-05-09)** — **#10a + #10b 통합 표기 반영** (사업기획 v1.10·한장구조도 v1.7·항목별연계 v1.2·컨소시엄planB v2.0과 동기화). #5 KPI 매트릭스 다이어그램의 *"with #10 split into #10a and #10b"* → *"#10 단일 항목, 4x4 grid 15개 항목"*. 다른 다이어그램의 #10a/#10b 직접 참조는 이미 v1.8에서 *항목 번호·기관 색상* 일괄 배제로 제거되어 추가 변경 없음.
>
> **v2.2 변경 (2026-05-09)** — **디지털 섀도우 *시각화·대시보드* → #10 → #1 이전 반영** (사업기획 v1.11·한장구조도 v1.8·항목별연계 v1.3·컨소시엄planB v2.1과 동기화). #9a ZONE ④ child box *"디지털 섀도우 (실시간 거울 동기화)"* → *"데이터 게이트웨이 (텔레매트리 ↔ 모델 상태 변환, 시각화 대시보드는 #1 backbone)"*. ZONE ④ 부제 *"통신·Sim2Real 변환·디지털 섀도우"* → *"통신·데이터 게이트웨이·Sim2Real 변환"*. #9b 캡션·legend·1차년도 합의 안건의 *"디지털 섀도우 동기"* → *"데이터 게이트웨이 + #1 디지털 섀도우 대시보드"*로 책임 명확화. #3 AI 사이클의 *"Sim2Real 전이 + 디지털 섀도우"* 표기는 *DT 전체 영역*을 의미하므로 변경 없음.
...
(#10 단일 항목, v2.0 통합), arranged in a 4x4 grid (15개 항목).
...
- 4개 영역으로 구성 — ① 장비 모델 ② 작업 환경 ③ 시나리오·계획 ④ DT 인터페이스 SW (통신·데이터 게이트웨이·Sim2Real 변환). *디지털 섀도우 시각화·대시보드는 #1 backbone 영역 — 별도 표기 없음*
...
[ZONE ④ — RIGHT SIDE, vertical panel] "DT 인터페이스 SW (외부 시스템 facing)"
  ...
  (ALL DT-side software — no 실차 hardware):
    ...
    [데이터 게이트웨이<br/>텔레매트리 ↔ 모델 상태 변환<br/>(시각화 대시보드는 #1 backbone)]
...
- DT는 *중앙 허브*로서 sim 실행·데이터 생성·디지털 섀도우 *시각화*·정책 평가 담당. 통신 인프라(#10)는 데이터 흐름까지, 섀도우 시각화는 DT 플랫폼 backbone(#1)에서 동작
...
[EAST — RIGHT CLUSTER] "실차 시스템 (실세계 하드웨어 + 시험 인프라)"
  ...
  Bidirectional arrows between this cluster and DT center:
    ...
    실차 텔레메트리 → DT : "실차 측정값 (#1 디지털 섀도우 대시보드 입력)" (THICK red arrow)
...
- 1차년도 시스템 통합 명세서 작성 시 **3개 인터페이스 합의 우선순위**:
  ...
  - ⓒ DT ↔ 실차 (통신 프로토콜·데이터 게이트웨이·정책 배포 방식) — *국책과제 TRL 3~5는 OTA 불필요. 디지털 섀도우 시각화는 #1 backbone에서 별도*
...
| v2.1 | 2026-05-09 | **#10a + #10b 통합 표기 반영** — 사용자 지적 *"#10a/#10b 분할은 인위적, 본질은 실차→DT 데이터 전송 + 대시보드 동시 시청"*. 사업기획 v1.10·한장구조도 v1.7·항목별연계 v1.2·컨소시엄planB v2.0과 동기화. #5 KPI 매트릭스 다이어그램의 *"with #10 split into #10a and #10b"* → *"#10 단일 항목, 4x4 grid 15개 항목"*으로 변경. 다른 다이어그램의 #10a/#10b 직접 참조는 이미 v1.8에서 *항목 번호·기관 색상* 일괄 배제로 제거되어 추가 변경 없음. |
| v2.2 | 2026-05-09 | **디지털 섀도우 *시각화·대시보드* → #10 → #1 이전 반영** — 사용자 지적 *"가상 굴착기 자세 동기화·대시보드는 통신이 아니라 DT 플랫폼 역할"*. 사업기획 v1.11·한장구조도 v1.8·항목별연계 v1.3·컨소시엄planB v2.1과 동기화. #9a ZONE ④ child box *"디지털 섀도우 (실시간 거울 동기화)"* → *"데이터 게이트웨이 (텔레매트리↔모델 상태 변환)"*. ZONE ④ 부제 *"통신·Sim2Real 변환·디지털 섀도우"* → *"통신·데이터 게이트웨이·Sim2Real 변환"*. #9b 캡션·legend·1차년도 합의 안건에서 *"디지털 섀도우 동기"* → *"데이터 게이트웨이 + #1 디지털 섀도우 대시보드"*로 책임 명확화. R&R: #10은 데이터 흐름까지, #1은 가상 모델 매핑·시각화·대시보드. |
